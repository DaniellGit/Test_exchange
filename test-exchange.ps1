...
[int]$transportqueuehigh = 150							#Change this to set transport queue high threshold. Must be higher than warning threshold.
[int]$transportqueuewarn = 100							#Change this to set transport queue warning threshold. Must be lower than high threshold.
...
$reportemailsubject = "System Operations Status Daily Report - BEST Exchange Health Check"
				...
				if ($version -ne "Exchange 2003")
				{
					...
					if ($IsMB)
					{
						...
						elseif ($activedbs.count -gt 0 -or ($version -eq "Exchange 2007" -and $mbdbs.count -gt 0))
						{
							...
							if ($testmailflowresult -eq "成功" -or $testmailflowresult -eq $success)
							{
